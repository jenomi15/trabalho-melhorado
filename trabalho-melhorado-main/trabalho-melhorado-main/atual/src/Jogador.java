public  abstract class Jogador  implements Comparable<Jogador> , JogadorBase {
    ...
    protected int pulaProximaRodada;// ao inves de ser boolean , deve ser um numero , que se nao for diferente de 0 , o cara n joga
    private int soma;
   ...
   // }//
    ...
    public void pularProximaRodada() {
        this.pulaProximaRodada = this.pulaProximaRodada + 1 ;
    }
   ...
   // }
    ...
    @Override
    public void adicionarMoedas(JogadorBase jogador, int moedasCasaSimples) {
        ((Jogador) jogador).setPontuacaoEmMoedas(((Jogador) jogador).getPontuacaoEmMoedas() + moedasCasaSimples);
    }
    ...
    // Novo método para verificar itens com base na descrição
    public boolean possuiItem(String item) {
        return getDescricao().contains(item);
    }

    public void adicionarMoedas(int moedasCasaSimples) {
        this.pontuacaoEmMoedas += getMoedasCasaSimples();
    }
    
    public int getMoedasCasaSimples() {
        int moedas = 1; 
        if (possuiItem("com óculos escuros")) {
            moedas += 3;
        } 
        if (possuiItem("com moletom")) {
            moedas += 2;
        }
        if (possuiItem("com boné")) {
            moedas += 1;
        }
        return moedas;
    }
