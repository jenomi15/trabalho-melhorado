...
public class Tabuleiro {
    ...
    public void criacaoDoTabuleiro() {
        ...
        // Cria uma lista com todas as posições possíveis inicializadas com casas simples
        List<Casa> tabuleiroJogado = new ArrayList<>(Collections.nCopies(totalCasas, null));
        ...
        for (String tipo : tiposDeCasas) {
            ...
            if (quantidade > 0) {
                for (int i = 0; i < quantidade; i++) {
                    ...
                    while (numero < 1 || numero > totalCasas || tabuleiroJogado.get(numero - 1) != null) {
                        if (tabuleiroJogado.get(numero - 1) != null) {
                            System.out.println("Posição já ocupada. Digite uma nova posição para a casa " + (i + 1) + " do tipo " + tipo + ":");
                        } else {
                    ...
                    // Cria a casa e coloca na posição correta
                    Casa novaCasa;
                    ...
                    tabuleiroJogado.set(numero - 1, novaCasa); // Substitui a posição correta
                }
            }
        }
        ...
        for (int i = 0; i < totalCasas; i++) {
            if (tabuleiroJogado.get(i) == null) { // Apenas adiciona onde não há casas especiais
                Casa novaCasa = CasaFactory.criarCasa("simples", i + 1);
                tabuleiroJogado.set(i, novaCasa);
            }
        }
    ...
    public void TurnoDoJogo() {
        ...
        while (!jogoTerminou()) {
            for (int a = 0; a < jogadores.size(); a++) {
                ...
                // Remover a cor da casa anterior
                removerCorCasa(posicaoAntiga, jogadorAtual.getCor());
    
                // Atualizar a posição do jogador
                jogadorAtual.setNumeroCasa(novaPosicao);
    
                // Adicionar a cor na nova casa
                adicionarCorCasa(novaPosicao, jogadorAtual.getCor());
    
                // Aplicar regra da casa atual
                tabuleiroJogado.get(novaPosicao).aplicarRegra(jogadorAtual);
    
                // Atualizar e imprimir o tabuleiro
                imprimirTabuleiro();
    
                // Se o jogador deve jogar novamente
                if (jogadorAtual.isDeveJogarNovamente()) {
                    // Remover a cor da nova posição antes de jogar novamente
                    removerCorCasa(novaPosicao, jogadorAtual.getCor());
                    // O jogador joga novamente, não incrementa o índice
                    a--; 
                    // Garantir que o jogo continue após a jogada extra
                    continue;
                }
    
                // Verifica se o dado rolado foi igual
